<script lang="ts" context="module">
	...
	type MaterialEntry = {
		[K in keyof typeof material]: [K, (typeof material)[K]];
	}[keyof typeof material];
	type LightsEntry = {
		[K in keyof typeof lights]: [K, (typeof lights)[K]];
	}[keyof typeof lights];

	function updateMaterial(props: MaterialEntry[]) {
		Object.assign(material, Object.fromEntries(props) as Partial<typeof material>);
	}

	function updateLights(props: LightsEntry[]) {
		Object.assign(lights, Object.fromEntries(props) as Partial<typeof lights>);
	}
</script>
...
<script lang="ts">
	...
	let updateML = false;
	const dummyArr = [0, 0, 0] as const;
	const speed = 1;
	const rotationIntensity = 1;
	const floatIntensity = 1;
	const floatingRange = [-0.1, 0.1];

	let parentRef: Group | null = null;
	let groupRef: Group | null = null;
	const offset = Math.random() * 10000;
	let arm1: Object3D | null = null;
	let arm2: Object3D | null = null;
	let currentArm: Object3D | null = null;

	function isRenderableMaterial(material: unknown): material is MeshStandardMaterial {
		return (
	...
	const updateArmMaterial = (arm: Object3D | null, currentArm: Object3D | null) => {
		if (arm && currentArm === arm) {
			const color = new Color(material.color);

			arm.traverse((node: Object3D) => {
				if (
					node instanceof Mesh &&
					!Array.isArray(node.material) &&
					isRenderableMaterial(node.material)
				) {
	...
	onMount(() => {
		...
		void (async () => {
			...
			arm1 = a1.scene.children[0] ?? a1.scene;
			arm2 = a2.scene.children[0] ?? a2.scene;
	...
	function onScroll(scroll: number) {
		if (!groupRef || thresholds.length < 2) return;
	...
	useScroll(({ scroll }) => {
		...
		if (scroll < $thresholdsStore['light-start']) {
			...
			// 	ambientColor: '#0E0E0E'
			// });
			if (lights.lightsColor === '#44ACFF') return;

			// lights.light1Intensity.value = 0.35;
			// lights.light2Intensity.value = 0.15;
			// lights.lightsColor = '#44ACFF';
			// lights.ambientColor = '#44ACFF';
			// lights = lights;
